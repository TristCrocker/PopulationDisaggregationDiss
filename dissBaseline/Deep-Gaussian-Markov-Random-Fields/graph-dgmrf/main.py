...
def main():
    ...
    for iteration_i in range(config["n_iterations"]):
        ...
        if ((iteration_i+1) % config["val_interval"]) == 0:
            ...
            num_features = graph_y.x.shape[1]  # 779 features dynamically

            if val_samples.shape[-1] != num_features:
                val_samples = val_samples.unsqueeze(-1).expand(-1, -1, num_features)
            ...
            masked_loss = (val_samples - graph_y.x) ** 2
            masked_loss = masked_loss[:, val_mask]  # Apply mask correctly
