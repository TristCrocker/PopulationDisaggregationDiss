...
def vi_loss(dgmrf, vi_dist, graph_y, config):
    ...
    # print("vi_samples shape:", vi_samples.shape)
    # print("graph_y.x shape:", graph_y.x.shape)
    # print("graph_y.mask shape:", graph_y.mask.shape)
    ...
    num_features = graph_y.x.shape[1]  # This will be 779 in your case

    # Dynamically reshape vi_samples to match graph_y.x
    if vi_samples.shape[-1] != num_features:  # Only expand if needed
        vi_samples = vi_samples.unsqueeze(-1).expand(-1, -1, num_features)
    
    masked_loss = (vi_samples - graph_y.x) ** 2  # Ensure tensor dimensions match
    masked_loss = masked_loss[:, graph_y.mask]  # Apply mask correctly
    l5 = -(1./(2. * utils.noise_var(config) * config["n_training_samples"])) * torch.sum(masked_loss)
